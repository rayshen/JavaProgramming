   ...
   [self dismissViewControllerAnimated:YES completion:^
    {
        ...
        [NSThread detachNewThreadSelector:@selector(toLogin) toTarget:self withObject:nil];
    }];
}

-(void)toLogin{
    NSLog(@"被执行");
    NSString *loginurl=[NSString stringWithFormat:@"%@&name=%@&pwd=%@",resultcode,[Personinfo getaccount],[Personinfo getpassword]];
    [PersonConnection loginwithQRcode:loginurl];
}
